...
impl ChatWidget {
    ...
    fn apply_explore_slash_command(&mut self) -> bool {
        if !self.collaboration_modes_enabled() {
            self.add_info_message(
                "Collaboration modes are disabled.".to_string(),
                Some("Enable collaboration modes to use /explore.".to_string()),
            );
            return false;
        }
        if let Some(mask) = collaboration_modes::explore_mask(self.model_catalog.as_ref()) {
            self.set_collaboration_mask(mask);
            true
        } else {
            self.add_info_message(
                "Explore mode unavailable right now.".to_string(),
                /*hint*/ None,
            );
            false
        }
    ...
    pub(super) fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Explore => {
                self.apply_explore_slash_command();
            }
            SlashCommand::Goal => {
    ...
    fn dispatch_prepared_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Explore if !trimmed.is_empty() => {
                if !self.apply_explore_slash_command() {
                    return;
                }
                let user_message = self.prepared_inline_user_message(
                    args,
